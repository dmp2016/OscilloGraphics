...
void putPixel(char x, char y, bool cl)
{
  if (x >= 0 && x < C_SCREEN_WIDTH && y >= 0 && y < 8)
    if (cl)
      drawTarget[x] |= 1 << y;
    else
      drawTarget[x] &= !(1 << y);
...
void printText(char* text, int ofs = 0)
{
  clearScreen();
  int sl = strlen(text);
  int j = 0, q = 0;
...
void clearIndicator() {
  indicator[6] = indicator[4] = indicator[3] = indicator[1] = indicator[0] = 48;
}
